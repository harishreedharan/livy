...
import com.cloudera.hue.livy.spark.client.{JobFailed, JobResult, JobCompleted}
...
class ClientSession(val sessionId: Int, createRequest: CreateClientRequest) extends Session {
  ...
  private def performOperation(m: (LivyClient => concurrent.Future[Serializable])): Long = {
